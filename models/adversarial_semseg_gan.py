...
def build_gan_one_optimizers(generator, discriminator, img_shape=(1, 28, 28), dropout_rate=0.25, l2_reg=0.):
    gan_input = Input(shape=img_shape)
    H_gen = generator(gan_input)
    gan_V = discriminator([H_gen, gan_input])
...
def build_gan_two_optimizers(generator, discriminator, img_shape=(1, 28, 28), dropout_rate=0.25, l2_reg=0.):
    gan_input = Input(shape=img_shape)
    H_gen = generator(gan_input)
    gan_V = discriminator([H_gen, gan_input])
    model = Model(input=[gan_input], output=[H_gen, gan_V], name='dcgan')
