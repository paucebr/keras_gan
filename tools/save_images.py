...
def my_label2rgb_adv(labels, colors, bglabel=None, bg_color=(0., 0., 0.)):
    w = labels.shape[0]
    h = labels.shape[1]
    output = np.zeros((w, h, 3,), dtype=np.float64)
    aux = np.argmax(labels, axis=2)
    
    for i in range(len(colors)):
        if i != bglabel:
            output[(aux == i).nonzero()] = colors[i]
    if bglabel is not None:
        output[(aux == bglabel).nonzero()] = bg_color
    return output
...
# Converts a label mask to RGB to be shown and overlaps over an image
def my_label2rgboverlay_adv(labels, colors, image, bglabel=None,
                        bg_color=(0., 0., 0.), alpha=0.2):
    image_float = gray2rgb(img_as_float(rgb2gray(image)))
    label_image = my_label2rgb_adv(labels, colors, bglabel=bglabel,
                               bg_color=bg_color)
    output = image_float * alpha + label_image * (1 - alpha)
    return output
...
# Save 3 images (Image, mask and result)
def save_img3_adv(image_batch, mask_batch, output, out_images_folder, epoch,
             color_map, classes, tag, void_label, n_legend_rows=1):
    #print('output shape: ' + str(output.shape))
    #print('Mask shape: ' + str(mask_batch.shape))
    output[(mask_batch == void_label).nonzero()] = void_label
    ...
    for j in range(output.shape[0]):
        ...
        #img = image_batch[j].transpose((1, 2, 0))
        label_out = my_label2rgb_adv(output[j], bglabel=void_label,
                                 colors=color_map)
        label_mask = my_label2rgboverlay_adv(mask_batch[j], colors=color_map,
                                         image=img, bglabel=void_label,
                                         alpha=0.3)
        label_overlay = my_label2rgboverlay_adv(output[j], colors=color_map,
                                            image=img, bglabel=void_label,
                                            alpha=0.3)
